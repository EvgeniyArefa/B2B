...
def calendar(request, country=0, negotitator=0, company=0):
    answer = dict()
    ...
    if 'negotitator' in request.GET:
        negotitator = request.GET['negotitator']
        negotitator = separation(negotitator)
        if negotitator == None:
            context = {
    ...
    if country == 0:
        if negotitator == 0:
            ...
            if company == 0:
                agreements_all = Agreement.objects.filter\
                    (negotiator__id__in=negotitator)
            else:
                agreements_all = Agreement.objects.filter\
                    (company__id__in=company, negotiator__id__in=negotitator)
    else:
        if negotitator == 0:
            if company == 0:
                agreements_all = Agreement.objects.filter\
